...
__attribute__((noinline)) void break_before() {
	__asm__ volatile("");
}

__attribute__((noinline)) void break_after() {
	__asm__ volatile("");
}

__attribute__((section("__TEST,__foo"), noinline)) static int my_own_function(int x) {
	return x + 4;
}
