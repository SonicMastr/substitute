...
#define DO_DECL_HTAB_KEY_(name, key_ty, func_decl)                               \
	func_decl void *__htab_key_lookup_##name(struct htab_internal *restrict hi,  \
											 const key_ty *restrict key,         \
											 size_t entry_size,                  \
											 bool add);                          \
	func_decl void __htab_key_removeat_##name(struct htab_internal *restrict hi, \
											  void *op,                          \
											  size_t entry_size);                \
	func_decl void __htab_key_memset_##name(void *ptr, size_t size);             \
	func_decl bool __htab_key_is_null_##name(const key_ty *bucket);              \
	func_decl void __htab_key_resize_##name(struct htab_internal *restrict hi,   \
											size_t size,                         \
											size_t entry_size)

#define IMPL_HTAB_KEY(name,		 /* size_t hash_func(const ty *) - hash.                                     \
								  * May be a macro. */                                                       \
					  hash_func, /* bool eq_func(const ty *stored, const ty *queried) - check whether the    \
								  * stored key is equal to the requested key (which is assumed to be valid). \
								  * May be a macro. */                                                       \
					  eq_func,	 /* bool null_func(const ty *) - check whether the stored key is a           \
								  * special invalid marker.                                                  \
								  * May be a macro. */                                                       \
					  null_func, /* uint8_t nil_byte - which byte to memset to initially.                    \
...
#define DO_IMPL_HTAB_KEY(name, key_ty, hash_func, eq_func, null_func, nil_byte)                                                                \
	...
	void __htab_key_removeat_##name(struct htab_internal *restrict hi,                                                                         \
		...
		while (1) {                                                                                                                            \
			...
			bool cf_after_hole /* cyclically */ = hole <= cur ? (hole < cur_chain_first) : (cur < cur_chain_first && cur_chain_first <= hole); \
			if (!cf_after_hole) {                                                                                                              \
...
#define DECL_HTAB(                                             \
	name, /* The name parameter to DECL_HTAB_KEY */            \
	key_name,                                                  \
	value_ty)                                                  \
...
#define HTAB_STORAGE_INIT(hs, name)                                                 \
	do {                                                                            \
