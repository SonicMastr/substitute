...
static bool oscf_try_dir(const char *dir, const char *arch,
	...
	if (lsi->nlistOffset > dch->localSymbolsSize ||
		lsi->nlistCount > (dch->localSymbolsSize - lsi->nlistOffset) / sizeof(substitute_sym) ||
	...
	if (!(lses = malloc(lses_size)))
		goto fail;
	if (pread(fd, lses, lses_size, dch->localSymbolsOffset + lsi->entriesOffset) != lses_size) {
		free(lses);
		goto fail;
	}
