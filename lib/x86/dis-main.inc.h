...
got_bits:
	...
	if (bits & I_JIMM_ONLY) {
		...
		P(branch)
		(ctx, ctx->base.pc + ctx->base.op_size + imm,
		...
		if (DIS_MAY_MODIFY && ctx->base.modify) {
			...
			switch (imm_size) {
			case 1:
				*(int8_t *)new_imm_ptr = new_imm;
				break;
			case 2:
				*(int16_t *)new_imm_ptr = new_imm;
				break;
			case 4:
				*(int32_t *)new_imm_ptr = new_imm;
				break;
			default:
				__builtin_abort();
			}
		...
		P(pcrel)
		(ctx, ctx->base.pc + ctx->base.op_size + disp, info);
		if (DIS_MAY_MODIFY && ctx->base.modify) {
			uint8_t *new_op = ctx->base.newop;
			memcpy(new_op, orig, ctx->base.op_size);
