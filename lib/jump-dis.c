...
static INLINE UNUSED void jump_dis_pcrel(struct jump_dis_ctx *ctx, uint_tptr dpc,
										 UNUSED struct arch_pcrel_info info) {
	ctx->bad_insn = dpc >= ctx->pc_patch_start && dpc < ctx->pc_patch_end;
}

static INLINE UNUSED void jump_dis_indirect_call(UNUSED struct jump_dis_ctx *ctx) {
}
...
static NOINLINE UNUSED void jump_dis_branch(struct jump_dis_ctx *ctx, uint_tptr dpc, int cc) {
