...
static NOINLINE UNUSED void transform_dis_data(struct transform_dis_ctx *ctx, unsigned o0, unsigned o1,
	...
	if (out_reg == 15) {
		...
		for (int i = 0; i < 4; i++)
			if (newval[i] == 15)
				newval[i] = scratch;
		ctx->write_newop_here = *codep;
		*codep += ctx->base.op_size;
...
static NOINLINE UNUSED void transform_dis_pcrel(struct transform_dis_ctx *ctx, uint_tptr dpc,
...
static NOINLINE UNUSED void transform_dis_branch(struct transform_dis_ctx *ctx, uint_tptr dpc, int cc) {
#ifdef TRANSFORM_DIS_VERBOSE
